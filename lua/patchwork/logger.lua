---@brief Centralized logger for patchwork (Claude Code IDE bridge).
-- Provides level-based logging.
---@module "patchwork.logger'
local M = {}
...
---@param plugin_config PatchworkConfig The configuration table (e.g., from patchwork.init.state.config).
function M.setup(plugin_config)
  ...
  if conf and conf.log_level and level_values[conf.log_level] then
    ...
    vim.notify(
      "patchwork Logger: Invalid or missing log_level in configuration (received: "
...
local function log(level, component, message_parts)
  ...
  vim.schedule(function()
    if level == M.levels.ERROR then
      vim.notify(prefix .. " " .. message, vim.log.levels.ERROR, { title = "patchwork Error" })
    elseif level == M.levels.WARN then
      vim.notify(prefix .. " " .. message, vim.log.levels.WARN, { title = "patchwork Warning" })
    else
...
---@param level_name PatchworkLogLevel The level name ("error", "warn", "info", "debug", "trace")
