...
---@module "patchwork.selection'
local M = {}

local logger = require("patchwork.logger")
local terminal = require("patchwork.terminal")
...
function M._create_autocommands()
  local group = vim.api.nvim_create_augroup("PatchworkSelection", { clear = true })
...
function M._clear_autocommands()
  vim.api.nvim_clear_autocmds({ group = "PatchworkSelection" })
end
...
function M.update_selection()
  ...
  if current_mode == "v" or current_mode == "V" or current_mode == "\022" then
    ...
    if current_selection then
      ...
      if
        M.state.last_active_visual_selection
        and M.state.last_active_visual_selection.bufnr == current_buf
      then
        M.state.last_active_visual_selection = nil
      end
    end
...
function M.handle_selection_demotion(original_bufnr_when_scheduled)
  ...
  if
    ...
    and (
      current_mode_info.mode == "v"
      or current_mode_info.mode == "V"
      or current_mode_info.mode == "\022"
    )
  then
...
function M.get_visual_selection()
  ...
  local lsp_positions =
    calculate_lsp_positions(start_coords, end_coords, visual_mode, lines_content)
...
function M.send_at_mention_for_visual_selection(line1, line2)
  ...
  local main = require("patchwork")
  if not main.state.server then
  ...
  local success, error_msg = main.send_at_mention(file_path, start_line, end_line, "PatchworkSend")
