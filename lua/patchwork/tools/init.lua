...
function M.register_all()
  -- 9 MCP-exposed tools (get_diagnostics + close_tab intentionally skipped).
  M.register(require("patchwork.tools.open_file"))
  M.register(require("patchwork.tools.get_current_selection"))
  M.register(require("patchwork.tools.get_latest_selection"))
  M.register(require("patchwork.tools.get_open_editors"))
  M.register(require("patchwork.tools.get_workspace_folders"))
  M.register(require("patchwork.tools.open_diff"))
  M.register(require("patchwork.tools.close_all_diff_tabs"))
  M.register(require("patchwork.tools.check_document_dirty"))
  M.register(require("patchwork.tools.save_document"))
end
...
function M.register(tool_module)
  if not tool_module or not tool_module.name or not tool_module.handler then
    ...
    vim.notify(
      ...
      { title = "patchwork Tool Registration" }
    )
    return
  end
...
function M.handle_invoke(client, params) -- client needed for blocking tools
  ...
  if tool_data.requires_coroutine then
    ...
    require("patchwork.logger").debug(
      "tools",
      "Wrapping " .. tool_name .. " in coroutine for blocking behavior"
    )
    local co = coroutine.create(function()
      return tool_data.handler(input)
    end)

    require("patchwork.logger").debug("tools", "About to resume coroutine for " .. tool_name)
    ...
    if coroutine.status(co) == "suspended" then
      require("patchwork.logger").debug(
        "tools",
        "Coroutine is suspended - tool is blocking, will respond later"
      )
  ...
  if not pcall_success then
    ...
    if
      type(handler_return_val1) == "table"
      and handler_return_val1.code
      and handler_return_val1.message
    then
  ...
  if handler_return_val1 == false then
    ...
    if
      type(err_val_from_handler) == "table"
      and err_val_from_handler.code
      and err_val_from_handler.message
    then
