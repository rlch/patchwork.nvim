...
function M.process_handshake(request, expected_auth_token)
  ...
  local is_valid_upgrade, validation_payload =
    M.validate_upgrade_request(request, expected_auth_token) ---@type boolean, table|string
  if not is_valid_upgrade then
    ...
    local response =
      M.create_error_response(400, "Bad WebSocket upgrade request: " .. error_message)
    return false, response, nil
  end
  ...
  if not response then
    local error_response =
      M.create_error_response(500, "Failed to generate WebSocket handshake response")
