---@brief WebSocket server for patchwork (Claude Code IDE bridge)
local main = require("patchwork") -- Added for version access
local logger = require("patchwork.logger")
local tcp_server = require("patchwork.server.tcp")
local tools = require("patchwork.tools.init") -- Added: Require the tools module
...
---@param config PatchworkConfig Configuration options
...
function M.start(config, auth_token)
  ...
  local callbacks = {
    ...
    on_connect = function(client)
      ...
      local main_module = require("patchwork")
...
function M.stop()
  ...
  if _G.patchwork_deferred_responses then
    _G.patchwork_deferred_responses = {}
  end
...
function M._setup_deferred_response(deferred_info)
  ...
  if not _G.patchwork_deferred_responses then
    _G.patchwork_deferred_responses = {}
  end
  _G.patchwork_deferred_responses[tostring(co)] = response_sender
