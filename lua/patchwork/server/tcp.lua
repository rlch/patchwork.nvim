...
local client_manager = require("patchwork.server.client")
local utils = require("patchwork.server.utils")
...
---@param config PatchworkConfig Server configuration
...
function M.create_server(config, callbacks, auth_token)
  ...
  if not port then
    return nil,
      "No available ports in range " .. config.port_range.min .. "-" .. config.port_range.max
  end
...
function M._disconnect_client(server, client, code, reason)
  ...
  local on_disconnect_mt = on_disconnect_type == "table" and getmetatable(server.on_disconnect)
    or nil
  assert(
    on_disconnect_type == "function"
      or (on_disconnect_mt ~= nil and type(on_disconnect_mt.__call) == "function"),
    ...
    for _, client in pairs(server.clients) do
      if client.state == "connected" then
        ...
        if client_manager.is_client_alive(client, interval * 2) then
          ...
          require("patchwork.logger").info(
            "server",
            string.format(
              "Client %s keepalive timeout (%ds idle), closing connection",
              client.id,
              time_since_pong
            )
          )
