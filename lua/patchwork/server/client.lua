...
local frame = require("patchwork.server.frame")
local handshake = require("patchwork.server.handshake")
    ...
    if complete and request then
      ...
      client.tcp_handle:write(response_from_handshake, function(err)
        if err then
          logger.error(
            "client",
            "Failed to send handshake response to client " .. client.id .. ": " .. err
          )
        ...
        if success then
          ...
          logger.debug(
            "client",
            "Closing connection for client due to failed handshake:",
            client.id
          )
