---@brief [[
--- Lock file management for patchwork (Claude Code IDE bridge).
...
local function generate_auth_token()
  ...
  if not random_initialized then
    ...
    if vim.loop and (vim.uv or vim.loop).hrtime then
      seed = seed + ((vim.uv or vim.loop).hrtime() % 1000000)
    end
