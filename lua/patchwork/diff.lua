...
local logger = require("patchwork.logger")
...
local function get_autocmd_group()
  if not autocmd_group then
    autocmd_group = vim.api.nvim_create_augroup("PatchworkMCPDiff", { clear = true })
  end
...
local function find_claude_terminal_window()
  local terminal_ok, terminal_module = pcall(require, "patchwork.terminal")
...
local function display_terminal_in_new_tab()
  ...
  local existing_terminal_win = find_claude_terminal_window()
  ...
  if name_ok and mod_ok and linecount_ok then
    if
      (initial_name == nil or initial_name == "")
      and initial_modified == false
      and initial_linecount <= 1
    then
  ...
  if
    config
    and config.diff_opts
    and type(config.diff_opts.hide_terminal_in_new_tab) == "boolean"
  then
...
---@param user_config PatchworkConfig The configuration passed from init.lua
...
local function create_temp_file(content, filename)
  local base_dir_cache = vim.fn.stdpath("cache") .. "/patchwork_diffs"
  ...
  if mkdir_ok_cache then
    ...
    local base_dir_temp = vim.fn.stdpath("cache") .. "/patchwork_diffs_fallback"
    local mkdir_ok_temp, mkdir_err_temp = pcall(vim.fn.mkdir, base_dir_temp, "p")
    if not mkdir_ok_temp then
      local err_to_report = mkdir_err_temp
        or mkdir_err_cache
        or "unknown error creating base temp dir"
...
local function cleanup_temp_file(tmp_file)
  if tmp_file and vim.fn.filereadable(tmp_file) == 1 then
    ...
    if vim.fs and type(vim.fs.remove) == "function" then
      ...
      if not ok_file then
        vim.notify(
          "patchwork: Error removing temp file " .. tmp_file .. ": " .. tostring(err_file),
          vim.log.levels.WARN
        )
      end
      ...
      if not ok_dir then
        vim.notify(
          "patchwork: Error removing temp directory " .. tmp_dir .. ": " .. tostring(err_dir),
          vim.log.levels.INFO
        )
      end
    else
      ...
      vim.notify(
        "patchwork: Cannot perform standard cleanup: "
          .. reason
          .. ". Affected file: "
          .. tmp_file
      ...
      if not os_ok then
        vim.notify(
          "patchwork: Fallback os.remove also failed for file "
            .. tmp_file
            .. ": "
            .. tostring(os_err),
          vim.log.levels.ERROR
...
local function choose_original_window(
  target_win,
  old_file_path,
  is_new_file,
  ...
  local is_empty_buffer = current_buf_path == ""
    and vim.api.nvim_buf_get_option(current_buf, "modified") == false
...
---@param original_win NvimWin
---@param original_buf NvimBuf|nil
...
local function setup_new_buffer(
  ...
  vim.b[new_buf].patchwork_diff_tab_name = tab_name
  vim.b[new_buf].patchwork_diff_new_win = new_win
  vim.b[new_buf].patchwork_diff_target_win = target_win_for_meta
  ...
  if config and config.diff_opts and config.diff_opts.keep_terminal_focus then
    vim.schedule(function()
      ...
      local terminal_win = find_claude_terminal_window()
...
function M._open_native_diff(old_file_path, new_file_path, new_file_contents, tab_name)
  ...
  local cleanup_group = vim.api.nvim_create_augroup("PatchworkDiffCleanup", { clear = false })
...
---@param new_window number? New diff window ID (for WinClosed rejection)
---@return table List of autocmd IDs
local function register_diff_autocmds(tab_name, new_buffer, new_window)
  local autocmd_ids = {}
  ...
  -- WinClosed: catches :q on the diff window even if the buffer survives in another window.
  -- Belt-and-suspenders with bufhidden=wipe so a misconfigured buffer still rejects on close.
  if new_window then
...
function M._create_diff_view_from_window(
  ...
  local choice =
    choose_original_window(target_window, old_file_path, is_new_file, terminal_win_in_new_tab)
  ...
  if is_new_file and choice.reused_buf and vim.api.nvim_buf_is_valid(choice.reused_buf) then
    ...
    original_buffer =
      load_original_buffer(original_window, old_file_path, is_new_file, existing_buffer)
  end
...
function M._cleanup_diff_state(tab_name, reason)
  ...
  if diff_data.created_new_tab then
    ...
    if
      diff_data.original_tab_number and vim.api.nvim_tabpage_is_valid(diff_data.original_tab_number)
    then
    ...
    if diff_data.new_tab_number and vim.api.nvim_tabpage_is_valid(diff_data.new_tab_number) then
      ...
      if
        diff_data.original_tab_number
        and vim.api.nvim_tabpage_is_valid(diff_data.original_tab_number)
      then
        pcall(vim.api.nvim_set_current_tabpage, diff_data.original_tab_number)
      end
    else
      ...
      if
        ...
        and current_tab ~= diff_data.original_tab_number
      then
...
function M._setup_blocking_diff(params, resolution_callback)
  ...
  -- Inline (VSCode/GitHub-style unified) layout takes a separate path; the
  -- side-by-side `:diffthis` plumbing below doesn't apply.
  if config and config.diff_opts and config.diff_opts.layout == "inline" then
    return M._setup_inline_diff(params, resolution_callback)
  end
  ...
  local setup_success, setup_error = pcall(function()
    ...
    if old_file_exists then
      ...
      if is_dirty then
        error({
          ...
          data = "Please save (:w) or discard (:e!) changes to "
            .. params.old_file_path
            .. " before creating diff",
        })
      end
    end
    ...
    local new_unique_name = is_new_file and (tab_name .. " (NEW FILE - proposed)")
      or (tab_name .. " (proposed)")
    ...
    -- Wipe on hide so :q in the diff window fires BufWipeout and triggers rejection.
    -- Without this, 'hidden' keeps the buffer loaded silently after :q.
    ...
    local autocmd_ids = register_diff_autocmds(tab_name, new_buffer, diff_info.new_window)

    -- Buffer-local 'q' to reject the diff explicitly (matches fugitive/oil-style quit).
    pcall(vim.api.nvim_buf_set_keymap, new_buffer, "n", "q", "", {
      noremap = true,
      silent = true,
      desc = "Reject patchwork diff",
      callback = function()
        M.deny_current_diff()
      end,
    })
...
function M.open_diff_blocking(old_file_path, new_file_path, new_file_contents, tab_name)
  ...
  local success, err = pcall(M._setup_blocking_diff, {
    ...
    if resume_success then
      ...
      if _G.patchwork_deferred_responses and _G.patchwork_deferred_responses[co_key] then
        _G.patchwork_deferred_responses[co_key](resume_result)
        _G.patchwork_deferred_responses[co_key] = nil
      else
      ...
      local co_key = tostring(co)
      if _G.patchwork_deferred_responses and _G.patchwork_deferred_responses[co_key] then
        _G.patchwork_deferred_responses[co_key]({
          error = {
            code = -32603,
...
function M.accept_current_diff()
  ...
  local tab_name = vim.b[current_buffer].patchwork_diff_tab_name
...
-- ---------------------------------------------------------------------------
-- Inline (VSCode/GitHub-style) diff renderer
--
-- Single read-only buffer with unified diff: removed lines highlighted as
-- DiffDelete, added lines as DiffAdd, intra-line character diffs as DiffText.
-- Sign column shows +/-, statuscolumn shows dual line numbers (old | new).
--
-- Built on `vim.diff()` (native since Neovim 0.9). No external plugin deps.
-- Theme integration: uses the standard Diff* highlight groups your colorscheme
-- already styles, so the diff matches your editor look without extra config.
--
-- Inspired by the inline layout in Cannon07/code-preview.nvim (MIT) — design
-- ported, code is independent.
-- ---------------------------------------------------------------------------
...
local function _split_contents(contents)
  local lines = vim.split(contents or "", "\n", { plain = true })
  -- Strip trailing empty line if content ended in \n
  if #lines > 0 and lines[#lines] == "" then
    table.remove(lines, #lines)
  end
  return lines
...
--- Compute char-level common prefix/suffix between two strings.
--- Returns {prefix_len, old_diff_end, new_diff_end}: the slice
--- old:sub(prefix_len+1, old_diff_end) and new:sub(prefix_len+1, new_diff_end)
--- are the parts that differ.
local function _intra_line_diff(old, new)
  local prefix = 0
  local n = math.min(#old, #new)
  while prefix < n and old:byte(prefix + 1) == new:byte(prefix + 1) do
    prefix = prefix + 1
  end
  local sfx = 0
  while sfx < (n - prefix) and old:byte(#old - sfx) == new:byte(#new - sfx) do
    sfx = sfx + 1
  end
  return prefix, #old - sfx, #new - sfx
end

--- Build the unified inline display lines and per-line metadata.
---@param old_lines string[]
---@param new_lines string[]
---@return string[] display_lines  Final rendered lines (with leading +/-/space)
---@return table[] line_meta       Per-display-line metadata
local function _build_inline_display(old_lines, new_lines)
  ...
  for _, hunk in ipairs(hunks) do
    local h_old_start, h_old_count, h_new_start, h_new_count = hunk[1], hunk[2], hunk[3], hunk[4]
    -- vim.diff uses 1-indexed line numbers; a count of 0 means insertion-at-position
    local del_start = h_old_count > 0 and h_old_start or (h_old_start + 1)
    local add_start = h_new_count > 0 and h_new_start or (h_new_start + 1)

    -- Context before this hunk
    while oi < del_start and ni < add_start do
      emit(" " .. (old_lines[oi] or ""), { type = "context", old_lnum = oi, new_lnum = ni })
      oi, ni = oi + 1, ni + 1
    end

    -- Removed lines
    for k = 0, h_old_count - 1 do
      local lnum = del_start + k
      emit("-" .. (old_lines[lnum] or ""), { type = "del", old_lnum = lnum, new_lnum = nil })
    end
    -- Added lines
    for k = 0, h_new_count - 1 do
      local lnum = add_start + k
      emit("+" .. (new_lines[lnum] or ""), { type = "add", old_lnum = nil, new_lnum = lnum })
    end

    oi = del_start + h_old_count
    ni = add_start + h_new_count
  end
...
--- Statuscolumn callback. Returns a string for the current line in the inline
--- diff buffer with two columns: old line number | new line number.
--- Width is computed once per buffer.
---@param buf integer
---@return string
function M.inline_statuscolumn(buf)
  local data = buf_inline_data[buf]
  if not data then
    return ""
  end
  local lnum = vim.v.lnum
  local meta = data.line_meta[lnum]
  if not meta then
    return "  "
  end
  local w = data.col_width
  ...
  local sign = meta.type == "add" and "+" or (meta.type == "del" and "-" or " ")
  return string.format("%s %s│%s ", sign, pad(meta.old_lnum), pad(meta.new_lnum))
end

local function _apply_inline_highlights(buf, display_lines, line_meta)
  vim.api.nvim_buf_clear_namespace(buf, inline_ns, 0, -1)

  -- Pair adjacent del/add runs to compute char-level intra-line highlights.
  -- A run is contiguous "-" lines followed by contiguous "+" lines.
  local i = 1
  while i <= #line_meta do
    local m = line_meta[i]
    if m.type == "del" then
      -- Collect del run
      local del_start = i
      while line_meta[i] and line_meta[i].type == "del" do
        i = i + 1
      end
      local del_end = i - 1
      -- Collect add run immediately following
      local add_start = i
      while line_meta[i] and line_meta[i].type == "add" do
        i = i + 1
      end
      local add_end = i - 1

      -- Whole-line backgrounds
      for k = del_start, del_end do
        vim.api.nvim_buf_set_extmark(buf, inline_ns, k - 1, 0, {
          end_row = k,
          end_col = 0,
          hl_group = "DiffDelete",
          hl_eol = true,
      ...
      -- Pairwise intra-line: only when del/add counts match — heuristic for "edited" hunks.
      if (del_end - del_start) == (add_end - add_start) then
        for off = 0, del_end - del_start do
          local di, ai = del_start + off, add_start + off
          local old_text = display_lines[di]:sub(2) -- strip leading "-"
          local new_text = display_lines[ai]:sub(2) -- strip leading "+"
          local prefix, old_e, new_e = _intra_line_diff(old_text, new_text)
          if old_e > prefix then
            vim.api.nvim_buf_set_extmark(buf, inline_ns, di - 1, 1 + prefix, {
              end_col = 1 + old_e,
              hl_group = "DiffText",
              priority = 200,
            })
          end
          if new_e > prefix then
            vim.api.nvim_buf_set_extmark(buf, inline_ns, ai - 1, 1 + prefix, {
              end_col = 1 + new_e,
              hl_group = "DiffText",
              priority = 200,
            })
    ...
    elseif m.type == "add" then
      -- Lone add (no preceding del) — pure insertion
      local add_start = i
      while line_meta[i] and line_meta[i].type == "add" do
        i = i + 1
      end
...
local function _find_main_window()
...
local function _next_change_jump(buf, dir)
  local data = buf_inline_data[buf]
  if not data then
    return
  end
  local cur = vim.api.nvim_win_get_cursor(0)[1]
  local n = #data.line_meta
  if dir > 0 then
    for lnum = cur + 1, n do
      local t = data.line_meta[lnum].type
      if
        (t == "add" or t == "del") and (lnum == 1 or data.line_meta[lnum - 1].type == "context")
      then
...
--- Accept the inline diff: write proposed content via the same code path the
--- side-by-side flow uses (so the resolution_callback fires identically).
---@param tab_name string
function M._inline_accept(tab_name)
  local diff_data = active_diffs[tab_name]
  if not diff_data or diff_data.status ~= "pending" then
    return
  end
  -- Build a transient buffer holding the proposed contents so we can reuse
  -- _resolve_diff_as_saved (which reads from a buffer).
  local src = vim.api.nvim_create_buf(false, true)
  vim.api.nvim_buf_set_lines(src, 0, -1, false, _split_contents(diff_data.new_file_contents))
  pcall(vim.api.nvim_buf_set_option, src, "eol", true)
  M._resolve_diff_as_saved(tab_name, src)
  pcall(vim.api.nvim_buf_delete, src, { force = true })
end

---@param tab_name string
function M._inline_reject(tab_name)
  local diff_data = active_diffs[tab_name]
  if not diff_data or diff_data.status ~= "pending" then
    return
  end
...
--- Inline diff main entry. Same contract as _setup_blocking_diff.
---@param params { old_file_path: string, new_file_path: string, new_file_contents: string, tab_name: string }
---@param resolution_callback fun(result: table)
function M._setup_inline_diff(params, resolution_callback)
  local tab_name = params.tab_name
  local old_lines = _read_lines(params.old_file_path)
  local new_lines = _split_contents(params.new_file_contents)
  local display, meta = _build_inline_display(old_lines, new_lines)

  -- Compute statuscolumn width based on max line number on either side
  local max_n = math.max(#old_lines, #new_lines)
  local col_width = math.max(#tostring(max_n), 1)

  -- Create the display buffer
  local buf = vim.api.nvim_create_buf(false, true)
  vim.api.nvim_buf_set_name(buf, tab_name .. " (diff)")
  ...
  -- Filetype for syntax highlighting (the leading +/- is part of each line,
  -- so the highlighting is approximate; theme-integrated diff bg dominates.)
  local ft = vim.filetype.match({ filename = params.new_file_path }) or ""
  if ft ~= "" then
    vim.bo[buf].filetype = ft
  end

  buf_inline_data[buf] = { line_meta = meta, col_width = col_width }

  -- Pick a window: vsplit alongside the user's main editor window
  local main_win = _find_main_window()
  local cur = vim.api.nvim_get_current_win()
  vim.api.nvim_set_current_win(main_win)
  vim.cmd("vsplit")
  local win = vim.api.nvim_get_current_win()
  vim.api.nvim_win_set_buf(win, buf)
  -- Don't steal focus from wherever the user was originally
  if vim.api.nvim_win_is_valid(cur) and cur ~= win then
    pcall(vim.api.nvim_set_current_win, cur)
  end

  -- Window-local visual setup
  vim.wo[win].number = false
  vim.wo[win].relativenumber = false
  vim.wo[win].wrap = false
  vim.wo[win].cursorline = true
  vim.wo[win].signcolumn = "no"
  vim.wo[win].statuscolumn = "%!v:lua.require'patchwork.diff'.inline_statuscolumn(" .. buf .. ")"
  vim.wo[win].winbar = "%#DiagnosticInfo# DIFF %* "
    .. (params.new_file_path or tab_name)
    .. "  %#Comment#[ga accept · gr/q reject · ]c [c next/prev]"

  _apply_inline_highlights(buf, display, meta)
  ...
  kmap("ga", function()
    M._inline_accept(tab_name)
  end, "patchwork: accept diff")
  kmap("gr", function()
    M._inline_reject(tab_name)
  end, "patchwork: reject diff")
  kmap("q", function()
    M._inline_reject(tab_name)
  end, "patchwork: reject diff (close)")
  kmap("]c", function()
    _next_change_jump(buf, 1)
  end, "Next hunk")
  kmap("[c", function()
    _next_change_jump(buf, -1)
  end, "Previous hunk")

  -- Also tag the buffer so other paths recognize it (parity with side-by-side)
  vim.b[buf].patchwork_diff_tab_name = tab_name
  vim.b[buf].patchwork_diff_layout = "inline"

  M._register_diff_state(tab_name, {
    old_file_path = params.old_file_path,
    new_file_path = params.new_file_path,
    new_file_contents = params.new_file_contents,
    new_buffer = buf, -- so cleanup wipes the inline buffer
    new_window = win, -- so cleanup closes the diff window
    target_window = nil,
    original_buffer = nil,
    original_buffer_created_by_plugin = false,
    target_window_created_by_plugin = true,
    ...
    resolution_callback = resolution_callback,
    result_content = nil,
    is_new_file = (vim.fn.filereadable(params.old_file_path) == 0),
    layout = "inline",
  })

  -- Jump cursor to first change for convenience
  for lnum, m in ipairs(meta) do
    if m.type == "add" or m.type == "del" then
